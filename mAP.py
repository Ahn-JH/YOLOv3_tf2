...
from core.argument import call_args
...
root_path=os.getcwd()
proj=cfg.YOLO.NAME
path=f'{root_path}/Projs/{proj}/mAP'
...
results_files_path = f"{path}/results"
...
ground_truth_files_list = glob.glob(f'{path}/ground-truth/*.txt')
...
for txt_file in ground_truth_files_list:
  ...
  if not os.path.exists(f'{path}/predicted/' + file_id + ".txt"):
...
predicted_files_list = glob.glob(f'{path}/predicted/*.txt')
...
for class_index, class_name in enumerate(gt_classes):
  ...
  for txt_file in predicted_files_list:
    ...
    file_id = os.path.basename(os.path.normpath(file_id))
    if class_index == 0:
      if not os.path.exists(f'{path}/ground-truth/' + file_id + ".txt"):
...
with open(results_files_path + f"/results.txt", 'w') as results_file:
...
if draw_plot:
  ...
  # end Plot title
  x_label = "Number of objects per class"
  eutput_path = results_files_path + "/Predicted Objects Info.png"
  to_show = False
  plot_color = 'forestgreen'
