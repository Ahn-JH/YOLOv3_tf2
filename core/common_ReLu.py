...
# MISH Activation function implemented

def mish(x):
    return x * tf.math.tanh(tf.math.softplus(x))

def convolutional_M(input_layer, filters_shape, downsample=False, activate=True, bn=True, name=None):
    if downsample:
        input_layer = tf.keras.layers.ZeroPadding2D(((1, 0), (1, 0)))(input_layer)
        padding = 'valid'
        strides = 2
    ...
    if bn: conv = tf.keras.layers.BatchNormalization(name=bn_name)(conv)
    if activate == True: conv = mish(conv)
    return conv

def Residual_network_M(input_layer, filters, i, Rname=None):
    prev = input_layer
    name = f'{Rname}/conv{i*2+1}' if Rname != None else None
    conv = convolutional_M(input_layer, filters_shape=(1, 1, filters // 2) , name=name)
    name = f'{Rname}/conv{i*2+2}' if Rname != None else None
    conv = convolutional_M(conv       , filters_shape=(3, 3, filters), name=name)
    Residual_ouput = tf.keras.layers.Add()([prev,conv])
    return Residual_ouput

def Residual_block_M(Residual_layer, filters, blocks, name=None):

    for i in range(blocks):
        Residual_layer = Residual_network_M(Residual_layer, filters, i, name)
