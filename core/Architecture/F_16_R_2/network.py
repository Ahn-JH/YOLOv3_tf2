...
def Darknet(name=None):
    ...
    x = common.convolutional(x,      filters_shape=(3, 3, 32), name='conv1', downsample=True)

    x = common.Residual_block(x, 32, 1, name="Residual1")
    x = common.convolutional(x,      filters_shape=(3, 3, 64), name='conv2', downsample=True)

    x = common.Residual_block(x, 64, 2, name="Residual2")
    x = common.convolutional(x,      filters_shape=(3, 3, 128), name='conv3', downsample=True)
    x = common.Residual_block(x, 128, 8, name="Residual3")

    route_2 = x

    x = common.convolutional(x, filters_shape=(3, 3, 256), name='conv4', downsample=True)
    x = common.Residual_block(x, 256, 8, name="Residual4")

    route_1 = x
    x = common.convolutional(x, filters_shape=(3, 3, 512), downsample=True, name='conv5')
    route_0 = common.Residual_block(x, 512, 4, name="Residual5")
