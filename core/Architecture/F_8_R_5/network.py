...
def Darknet(name=None):
    ...
    x = common.Residual_block(x, 32, 5, name="Residual2")
    x = common.convolutional(x,      filters_shape=(3, 3, 64), name='conv3', downsample=True)
    x = common.Residual_block(x, 64, 20, name="Residual3/route_2")

    route_2 = tf.convert_to_tensor(x, name = 'route_2')

    x = common.convolutional(x, filters_shape=(3, 3, 128), name='conv4', downsample=True)
    x = common.Residual_block(x, 128, 20, name="Residual4/route_1")

    route_1 = tf.convert_to_tensor(x, name = 'route_1')
    x = common.convolutional(x, filters_shape=(3, 3, 256), downsample=True, name='conv5')
    x = common.Residual_block(x, 256, 10, name="Residual5/route_0")
    route_0 = tf.convert_to_tensor(x, name = 'route_0')
